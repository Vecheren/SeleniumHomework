...
public class JQueryCalendar_Tests
{
    ...
    public void JQueryUI_ChooseDate_Success()
    {
        ...
        (driver as IJavaScriptExecutor).ExecuteScript(
            $"$('#datepicker').datepicker('setDate','{thisDatePlusAddedDays.ToString("MM/dd/yyyy", CultureInfo.InvariantCulture)}')");
        var selectedDate =
            (driver as IJavaScriptExecutor).ExecuteScript(
                "return $.datepicker.formatDate('yy-mm-dd', $('#datepicker').datepicker('getDate'))");
        Assert.AreEqual(thisDatePlusAddedDays.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture), selectedDate,
            $"Выбранная дата не равна текущей дате + {addedDaysAmount} дней");
    }
